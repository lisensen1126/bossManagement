    ...
    .section-layout
      .section-filter.is-horizontal
        .field-group.is-horizontal
          ...
          //- .field.is-narrow
          //-   custom-select(v-model="thirdState", :list="thirdClassList", nameField="category_name")
          .field.is-keyword
            label.label 关键字搜索
          ...
          table.table
            ...
            tbody(v-if="list.length")
              tr(v-for="(item, index) in list")
                ...
                td
                  .operation-td
                    span.operation-link( @click="goDetail(item.product_id)") 查看
                    //- span.operation-link( @click="addServiceStore(item.product_id)") 添加到商品库
        .tfoot(v-if="list.length")
          pagination(:pagination-data="paginationData", @input="paging")
      select-class(v-model="itemSelectorClass", v-if="itemSelectorClass",isCommodity="true", @resolve="itemSelectorResolve", :id="nowId")
</template>
...
export default {
  ...
  mixins: [enterpriseMixin, filterTools, imageScale, formateDate],
  components: {
    CustomInput,
    CustomSelect,
    EmptyPage,
  ...
  data () {
    const schema = {
      ...
      thirdClassList: [],
      list: [],
      sellingStates: [{ name: '全部', value: 'ALL' }, { name: '已启用', value: true }, { name: '已禁用', value: false }],
      paginationData: {
        current_page: 1,
        per_page: 20,
        last_page: 1,
  ...
  methods: {
    ...
    async fetchData () {
      ...
      try {
        const { response, result } = await this.goodCollectionList({
        ...
        if (response.status === 200 && result.code === 0) {
          result.data.forEach(candidate => (candidate.isSelected = false))
          ...
          this.$store.dispatch('showMessage', {
            type: 'failure',
            title: '获取商品列表失败',
            content: `商品异常，请稍后再试（${response.status}${result && result.code ? ' ' + result.code : ''}）`,
          })
        }
      } catch (err) {
        this.$store.dispatch('showMessage', {
          type: 'failure',
          title: '获取商品列表失败',
          content: `商品异常，请稍后再试（${err.response ? err.response.status : err}${err.result && err.result.code ? ' ' + err.result.code : ''}）`,
        })
      }
      this.isInitiated = true
    },
  ...
  beforeRouteEnter (to, from, next) {
    if (from.name === 'commodityCollectionDetail') {
      to.meta.isBack = true
    }
    next()
  },
