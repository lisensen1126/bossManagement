          ...
          .field.is-horizontal
            ...
            .control
              ...
              .sku-box(v-for="(sku, indexItem) in item.sku_data")
                ...
                .sku-txt.width-80(v-text="sku.is_enabled === 1 ? '已启用':'已禁用'")
                .set-push
                  span.operation-link(v-if="sku.is_enabled === 1 && sku.is_push === 2", @click="setPush(2, sku)") 取消主推
                  span.operation-link(v-if="sku.is_enabled === 1 && sku.is_push === 1", @click="setPush(1, sku)") 设为主推
                  span(v-if="sku.is_enabled === 2") --
      .btn-wrapper
        .button-group
          a.button.primary.large(v-if="item.is_sale === 2", @click="setShopStatus(1)") 上架
          a.button.primary.large(v-if="item.is_sale === 1", @click="setShopStatus(2)") 下架
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      * sku sku对象
      * */
      async setPush (type, sku) {
        let text = '设为主推'
        if (type === 2) text = '取消主推'
        try {
          const {response, result} = await this.setSkuPushApi({
            product_id: this.product_id - 0,
            sku_id: type === 2 ? 0 : sku.sku_id,
          })
        ...
        try {
          ...
          if (response.status === 200 && result.code === 0) {
            ...
            result.data.sku_data.forEach(function (item) {
              item.is_push = 1
              if (item.sku_id === result.data.push_sku) item.is_push = 2
              item.value = ''
      ...
      async setShopStatus (param) {
        try {
          let { response, result } = await this.commodityUpdateSale({
            product_ids: [this.item.product_id],
            is_sale: param,
          })
          if (response.status === 200) {
            if (result.code === 0) {
