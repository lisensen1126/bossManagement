...
<script>
  ...
  export default {
     ...
     */
    name: 'ServiceAdd',
    mixins: [validationMixin, drawApi, fileUploader],
    components: {
      CustomInput,
      UploadPic,
      CustomSelect,
    ...
    methods: {
      ...
      async getDetail () {
        try {
          ...
          const {result, response} = await this.getPosterDetailApi(params)
          if (response.status === 200) {
            if (result.code === 0) {
              this.item.poster_pic = result.data.cover
              this.item.name = result.data.name
              this.poster_type = this.type_list[result.data.type - 1]
            } else {
              this.$store.dispatch('showMessage', {
                type: 'failure',
                title: '获取详情失败',
      ...
      async saveData () {
        try {
          ...
          const {result, response} = await this.getPosterSaveApi(params)
          if (response.status === 200) {
            if (result.code === 0) {
              this.$store.dispatch('showMessage', {
                autoClose: true,
