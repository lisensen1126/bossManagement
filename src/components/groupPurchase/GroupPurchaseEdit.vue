<template lang="pug">
  .section-layout
    .content-wrapper
      ...
      .form-wrapper
        .field-group
          ...
          .field.is-horizontal
            label.label 发布渠道
            .control
              .switch(:class="{'disabled': isLoading}")
      ...
      .btn-wrapper
        .button-group
          a.button.primary.large(@click="inspectData", :class="{'disabled': isDisabled || isLoading}") 提交审核
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async editTuanGou () {
        try {
          const {response, result} = await this.updateTuanGouApi({
      ...
      async editTuanDan () {
        try {
          const {response, result} = await this.updateTuanDanApi({
            id: this.$route.query.id,
            name: this.info.name,
            category_id: this.info.category.category_id,
            service: [...this.info.service_list],
        ...
        try {
          ...
          if (response.status === 200) {
            if (result.code === 0) {
              ...
              this.info.service = result.data.service
              this.info.services_intro = result.data.services_intro
              this.info.max_sale_qty = result.data.max_sale_qty
              this.info.stock_qty = result.data.stock_qty
              this.info.saleDate = this.changeDateTime(result.data.sale_begin_date) + ' 至 ' + this.changeDateTime(result.data.sale_end_date)
              this.info.effectiveDate = this.changeDateTime(result.data.effective_begin_date) + ' 至 ' + this.changeDateTime(result.data.effective_end_date)
              this.info.head_images = result.data.head_images
              this.info.other_images = result.data.other_images
              this.info.product_intro = result.data.product_intro
              this.info.customer_intro = result.data.customer_intro
              ...
              this.categoryList.forEach(ele => {
                if (ele.category_id === result.data.category_id) {
                  this.info.category = ele
                }
              })
              let sum = 0
              result.data.service.forEach(ele => {
                ele.original_price = ele.original_price / 100
                ele.cost_price = ele.cost_price / 100
                ele.sell_price = ele.sell_price / 100
                sum += Number(ele.sell_price)
              })
              this.num_sum = sum
            } else {
    ...
    mounted () {
      ...
      if (this.$route.query.type === 1) {
        this.getTuanGouDetail()
      } else {
        this.getTuanDanDetail()
      }
    },
    validations: {
      info: {
