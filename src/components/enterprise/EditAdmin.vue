  ...
  .slide-layout
    .cus-box
      .slide-title 编辑管理员
        ...
        .field
          label.label 账号(手机号码)
          custom-input(v-model="addInfo.mobile", need-type='number', maxlength=11, placeholder="请输入账号(手机号码)", :validator="$v.addInfo.mobile", error-info="账号为手机号码",)
        .field
          label.label 密码
          .gray ******
        .field
      ...
      async getStoreList () {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            // 如果是编辑门店管理员，则需要将之前所属门店显示出来
            if (_this.editItem.store_id && _this.editItem.role_name === 'STORE_ADMIN') {
              this.storeList.forEach(item => {
                if (item.store_id === _this.editItem.store_id) {
                  _this.addInfo.store = item
                }
              })
            }
          } else {
