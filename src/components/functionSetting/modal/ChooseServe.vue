        ...
        .main-right
          ...
          .right-content
            .item-box(v-for="(item, index) in selected_item")
              input(type="checkbox", :id="item.product_id" :value="item", name="modal", hidden="true", v-model="selected_item", :checked='item.selected')
  ...
  export default {
    ...
    methods: {
      ...
      async getInfoList () {
        ...
        try {
          const { response, result } = await this.getSkuServe(params)
          if (response.status === 200 && result.code === 0) {
            // 列表数据赋值
            for (let i = 0; i < result.data.length; i++) {
              result.data[i].product_name = result.data[i].spu_name
              result.data[i].product_id = result.data[i].spu_id
              result.data[i].category_data = [result.data[i].category_name]
              result.data[i].product_pic = result.data[i].spu_pic
            }
