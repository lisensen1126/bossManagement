    ...
    .section-filter
      .field-group.is-horizontal
        .field.keyword-wrapper
          label.label 名称搜索
          custom-input(v-model="filterData.name", placeholder="请输入海报名称", @keyupEnter="applyFilter")
        .field
          label.label 海报类型
      ...
      .field-group.is-horizontal
        .field
          ...
          a.button.primary(@click="addPoster")
            span.add-icon
            span 添加宣传海报
    empty-page(v-if="list.length === 0", :tip="'当前没有海报'")
          ...
          tbody
            tr(v-for="(item, index) in list")
              td
                .simple-td {{item.name}}
              td
                .simple-td {{item.type === 1 ? '爆款推荐海报' : item.type === 2 ? '活动宣传海报' : ''}}
              td
                .simple-td {{Number(item.updated_at) | date('yyyy-MM-dd HH:mm','cn')}}
              td
                .simple-td.width-fixed
                  span(v-if="item.has_upload.preview_poster === 1") 海报预览图片 
                  span(v-if="item.has_upload.roll_screen_poster === 1") 易拉宝海报图片 
                  span(v-if="item.has_upload.regular_poster === 1") 常规海报图片 
                  span(v-if="item.has_upload.a4_poster === 1") A4海报图片 
                  span(v-if="item.has_upload.online_poster === 1") 线上宣传海报 
              td
                .simple-td
                  .switch
  ...
  export default {
    ...
    methods: {
      ...
      toggleSidePannel (item) {
        this.img_bg = [item.preview_poster]
        this.type = item.type
      ...
      async displayConfirm (index) {
        try {
          const { response, result } = await this.updateStatusPosterApi({
      ...
      * */
      async deleteApi (id) {
        try {
          const { result, response } = await this.deletePosterApi({
            id: id,
          })
      ...
      * */
      ...
      async fetchData () {
        try {
          const { response, result } = await this.fetchPosterListApi({
