      ...
      .form-wrapper
        ...
        .field-group
          .field.is-horizontal
            ...
            .control.upLoad-image
              upload-pic(v-model="item.poster_preview", img-type="item-preview", :need-default="false",img-id="item-preview", :max-size="10")
              .picture-description 建议上传尺寸为2267px*5101px的jpg/png格式文件，文件不能超过10M。
          .field.is-horizontal
            label.label 易拉宝海报图片
            .control.upLoad-image
              upload-pic(v-model="item.poster_rollup.thumbnail_image_url", img-type="item-rollup", :need-default="false", img-id="item-rollup", :max-size="60", :is-large="true", @input="resultData")
          ...
          .field.is-horizontal
            label.label A4海报图片
            .control.upLoad-image
              upload-pic(v-model="item.poster_a4.thumbnail_image_url", img-type="item-a4", :need-default="false", img-id="item-a4", :max-size="60", :is-large="true", @input="resultData2")
        ...
        .field-group
          .field.is-horizontal
            ...
            .control.upLoad-image
              upload-pic(v-model="item.poster_master.thumbnail_image_url", img-type="item-master", img-id="item-master", :max-size="2", :need-default="false", :is-large="true", @input="resultData3")
    ...
    methods: {
      ...
      async saveData () {
        ...
        try {
          ...
          const {result, response} = await this.updatePosterApi(params)
      ...
      async getDetail () {
        try {
          ...
          const {result, response} = await this.fetchPosterDetailApi(params)
          if (response.status === 200) {
            if (result.code === 0) {
              if (result.data.type === 1) {
                ...
                this.poster_type = null
              }
              this.item.name = result.data.name
              this.item.poster_preview = result.data.preview_poster.thumbnail_image_url
              this.item.poster_rollup = {...result.data.roll_screen_poster}
              this.item.poster_regular = {...result.data.regular_poster}
              this.item.poster_a4 = {...result.data.a4_poster}
              this.item.poster_master = {...result.data.online_poster}
