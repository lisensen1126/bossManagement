    ...
    .content-wrapper
      ...
      .order-box(v-if="info.store")
        ...
        dl(v-if="Number(info.store_random_coupon)>0")
          dt  门店立减金
          dd  ￥ {{(info.store_random_coupon/100).toFixed(2)}}
      .order-box
        .title 客户信息
        dl
          dt 客户昵称
      ...
      .order-box
        ...
        dl.cright
          ...
          dt.t-r
            ...
            div(v-if="Number(info.user_random_coupon)>0") - ￥{{(info.user_random_coupon/100).toFixed(2)}}
            div ￥{{info.final_amount}}
            div(v-if="cancelled_by > 4 && info.purchase_type - 1 === 1") ￥{{info.final_amount}}
      .btn-wrapper
        .button-group
