  ...
  modal-dialog(modal-scale="large")
    .modal-card(slot="card")
      ...
      .modal-body
        .form-content
          span 分类名称
          custom-input(v-model="name", placeholder="请输入分类名称", :validator="$v.name", error-info="名称不能为空", maxLength="10")
      .modal-footer.level
        .level-left
        .level-right
    ...
    methods: {
      ...
      * */
      async saveData (type) {
        ...
        try {
          const {response, result} = type === 1 ? await this.editBrandCategoryApi(params) : await this.addBrandCategoryApi(params)
