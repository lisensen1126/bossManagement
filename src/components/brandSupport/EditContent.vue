...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        try {
          ...
          if (result.code === 0 && response.status === 200) {
            ...
            result.data.forEach(item => {
              if (this.info.category_id === item.id) this.select_classify = item
            })
          } else {
      ...
      async getData () {
        try {
          const {response, result} = await this.detailContentApi({
            id: this.$route.query.id,
          })
          if (response.status === 200) {
            this.info = result.data
            this.info.title = result.data.name
            let status = true
            if (this.info.status === 2) status = false
            this.info.is_status = status
      ...
      async saveData () {
        ...
        try {
          const {response, result} = await _this.editContentApi(params)
          if (response.status === 200) {
            if (result.code === 0) {
              this.$store.dispatch('showMessage', {
