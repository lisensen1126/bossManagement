<template lang="pug">
  .upload-pic
    ...
    .tap-wrapper(v-if="!!imageUrl && !isDisabled")
      label(:for="componentId", v-text="defaultTip", v-if="!isDisabled")
      .action(@click="secondAction", v-if="hasSecondAction", v-text="secondActionTip")
    .tap-wrapper(v-if="isLoading") 加载中...
    input(type="file", :id="componentId", @change="commitImage", class="input-file", v-if="!isDisabled")
</template>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.componentId = this.imgId ? this.imgId + 'ip' : new Date().getTime() + 'ip'
    },
  }
</script>
