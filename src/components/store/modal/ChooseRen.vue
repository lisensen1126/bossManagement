<template lang="pug">
  modal-dialog(modal-scale="large", v-if="value")
    .modal-card(slot="card")
      .modal-header
        .modal-card-title 选择认证
        .close-button(@click="modalClose")
      .modal-body
        empty-page(v-if="!dataList.length && isInitiated", :tip="'暂无认证'")
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      existedIds () {
        return new Set(this.nowRen.map(candidate =>
    ...
    mounted () {
      this.dataList = this.processItemOptions(this.allRen)
      // this.processLimitItemOptions()
      if (this.dataList.length === this.nowRen.length) {
        this.isSelectedAll = true
      }
    },
  }
